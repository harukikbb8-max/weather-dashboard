...
| 機能 | 説明 |
|------|------|
| **動的空背景（太陽位置連動）** | 都市の緯度と時刻から太陽高度角を天文計算し、深夜→薄明→朝焼け→ゴールデンアワー→青空→夕焼け→薄明→深夜と背景グラデーションが連続的に遷移。7天気条件×7フェーズのカラーパレットを線形補間 |
| **太陽エレメント** | 太陽が時刻に応じて弧を描いて移動。地平線付近ではオレンジ/赤色に、高い位置では白/黄色に変化。曇り・雨・雷などの天気条件で減衰。ゴールデンアワー時にグローが強まる |
| **チャートホバー連動** | チャート上のデータポイントにマウスを乗せると、その時点の天気・時刻・太陽位置で空背景がリアルタイムに変化。星は薄明時に滑らかにフェードアウト |
...
│
├── lib/solar.ts              【天文計算層】
│   太陽高度角の簡易天文計算。緯度・日付・時刻から太陽位置を算出。
│   SkyBackground の背景グラデーションと太陽エレメントの配置に使用。
│
├── hooks/
...
| `SkyBackground` | `SkyBackground.tsx` | 動的空背景 | 太陽高度角に基づく連続グラデーション補間（7天気条件×7フェーズ）。太陽エレメントの弧移動。降水量連動の雨速度。ホバー中300ms/通常2000msのトランジション |
...
- **背景:** 太陽高度角に連動した連続グラデーション（7天気条件×7フェーズの色補間）
- **太陽:** 緯度・日付・時刻から天文計算した位置に弧を描いて移動する太陽エレメント
...
|--------|--------|------|-----------|
| 深夜 | 紺〜深紫 | 非表示 | 星の明滅 |
| 薄明 | 紫→ピンク | 微かに出現 | 星がフェードアウト |
| 朝焼け/夕焼け | オレンジ〜赤紫 | 地平線付近（暖色） | 大気散乱グロー |
| ゴールデンアワー | オレンジ→青 | 低い位置（強いグロー） | — |
| 日中 | 青空 | 弧の上部（白/黄） | — |
| 曇り/雨 | 彩度低下・暗い | 減衰〜非表示 | 雨/雪/霧 |
...
| 太陽位置を簡易天文計算で算出 | 外部ライブラリ不使用。太陽赤緯+時角→高度角の公式で日本の5都市（緯度26〜43°N）に十分な精度。季節による日照時間の変化も反映 |
| 空グラデーションを連続補間 | 昼/夜の2値切替→7フェーズのカラーパレット線形補間に変更。`lerpColor` で中間色を動的生成し、朝焼け〜真昼〜夕焼けの滑らかな遷移を実現 |
