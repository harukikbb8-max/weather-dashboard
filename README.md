...
| **チャートホバー連動** | チャート上のデータポイントにマウスを乗せると、その時点の天気・時刻で空背景がリアルタイムに変化。星は薄明時に滑らかにフェードアウト |
...
│
├── lib/solar.ts              【天文計算層】
│   太陽高度角の簡易天文計算。緯度・日付・時刻から太陽高度を算出。
│   SkyBackground の背景グラデーション遷移と大気散乱の位置に使用。
│
├── hooks/
...
| `SkyBackground` | `SkyBackground.tsx` | 動的空背景 | 太陽高度角に基づく連続グラデーション補間（7天気条件×7フェーズ）。大気散乱が太陽位置に追従。降水量連動の雨速度。ホバー中300ms/通常2000msのトランジション |
...
- **背景:** 太陽高度角に連動した連続グラデーション（7天気条件×7フェーズの色補間）+ 太陽位置に追従する大気散乱グロー
...
| 深夜 | 紺〜深紫 | 星の明滅 |
| 薄明 | 紫→ピンク | 星がフェードアウト |
| 朝焼け/夕焼け | オレンジ〜赤紫 | 大気散乱グロー |
| ゴールデンアワー | オレンジ→青 | 大気散乱グロー（強） |
| 日中 | 青空 | — |
| 曇り/雨 | 彩度低下・暗い | 雨/雪/霧 |
