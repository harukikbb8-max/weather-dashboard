...
| 2026-02-13 | 降水量0mmで雨エフェクトを非表示に | 天気コードがrain/thunderでも降水量が0なら雨を出さない。実データに忠実な演出 |
| 2026-02-13 | 太陽位置連動の空背景・太陽エレメント追加 | `lib/solar.ts` で太陽高度角を天文計算。7フェーズのカラーパレット線形補間で朝焼け→真昼→夕焼けの連続遷移。太陽divが弧を描いて移動し、天気条件で減衰 |
